...
#define RGB_FILENAME2   "bucovina.bmp"
#define RGB_FILENAME  "cat.bmp"
...
void add_neon_rgb24(const uint8_t *src, uint8_t *dst, uint32_t bytes, uint8_t factor)
{
   // TO DO
    uint32_t i = 0; 
    uint8x16_t vfactor = vdupq_n_u8(factor); 
    /* procesam 16 elemente in simultan*/ 
    for (; i + 16 <= bytes; i += 16) { 
        uint8x16_t v = vld1q_u8(src + i); 
        /* suma cu saturare */ 
        uint8x16_t vsum = vqaddq_u8(v, vfactor);  
        vst1q_u8(dst + i, vsum); 
    } 
...
void negative_neon_rgb24(const uint8_t *src, uint8_t *dst, uint32_t bytes)
{
    ...
    const uint8x16_t vone255 = vdupq_n_u8(255);
    for(; i + 16 <= bytes; i += 16){
        uint8x16_t v = vld1q_u8(src + i);
        uint8x16_t inv = vsubq_u8(vone255, v); 
        vst1q_u8(dst + i, inv); 
    }
...
void grayscale_neon_rgb24(const uint8_t *src, uint8_t *dst, uint32_t pixels) {
  // TO DO
    ...
    for (i = 0; i + 8 <= pixels; i += 8) { 
        const uint8_t *p = src + i*3; 
   // Încarcă 8 pixeli R, G, B intercalat din memorie 
        uint8x8x3_t rgb = vld3_u8(p); 
//Convertim fiecare canal de la 8 biți → 16 biți 
// pentru a putea înmulți fără overflow 
        uint16x8_t r16 = vmovl_u8(rgb.val[0]); 
        uint16x8_t g16 = vmovl_u8(rgb.val[1]); 
        uint16x8_t b16 = vmovl_u8(rgb.val[2]); 
        uint16x8_t sum = vaddq_u16(vaddq_u16(vmulq_n_u16(r16,77),  vmulq_n_u16(g16,150)), vmulq_n_u16(b16,29)); 
// Împărțim la 256 (>>8) și convertim u16 → u8 
        uint8x8_t gray = vshrn_n_u16(sum, 8); 
        uint8x8x3_t out; 
        out.val[0] = gray; 
        out.val[1] = gray; 
        out.val[2] = gray; 
//Scriem 8 pixeli grayscale (3 canale) în memorie 
        vst3_u8(dst + i*3, out); 
    } 
    for (; i < pixels; i++) { 
        const uint8_t *p = src + i*3; 
        uint8_t gray = (77*p[0] + 150*p[1] + 29*p[2]) >> 8; 
        dst[i*3 + 0] = gray;  dst[i*3 + 1] = gray; 
        dst[i*3 + 2] = gray; 
    } 
