...
//Declară două structuri XGpio pentru pinii de intrare și ieșire 
 XGpio input, output; 
 int button_data_msk = 0;//Variabilă pentru stocarea stării butoanelor 
...
//(LED-uri) 
    if(button_data == 0b00010){
      button_data_msk = 0b00101;
      XGpio_DiscreteWrite(&output, 1, button_data_msk);  
    }
    if(button_data == 0b00100){
      button_data_msk = 0b01010;
      XGpio_DiscreteWrite(&output, 1, button_data_msk);   
    }

    if(button_data == 0b01100){
        print("Am intrat in meniul unei aplicatii");
        print("Pentru a naviga in meniu apasati butoanele 2 si 3");
        print("Pentru a iesi apasati simultan butoanele 2 si 3");
        ...
        while(1){
            ...
            if(button_data == 0b00100 && option_c > 1){
                option_c--;
            }

            if(button_data == 0b01000 && option_c < 6){
                option_c++;
            }
