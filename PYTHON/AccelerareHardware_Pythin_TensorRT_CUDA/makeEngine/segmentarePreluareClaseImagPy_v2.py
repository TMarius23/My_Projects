...
import sys
import os 
# import cupy as cp # ELIMINAT: Folosim NumPy pentru stabilitate
...
INPUT_PATH = "Cars Moving On Road Stock Footage - Free Download.mp4" 

# Deschiderea globală a fișierului de log
try:
    # ATENȚIE: Fișierul rămâne deschis global. Închiderea se face în finally.
    fout = open("rezultate_box_clase_date.txt", "w")
except Exception as e:
    print(f"[ATENȚIE] Nu s-a putut deschide fișierul de log: {e}")
    fout = type('dummy', (object,), {'write': lambda self, s: None})()
...
def generate_unique_colors(num_classes):
    """ Generează culori pseudo-aleatoare unice (BGR) pentru fiecare clasă. """
    colors = []
...
def process_yolo_segmentation_output(outputs, orig_shape, input_shape, conf_threshold=0.35, iou_threshold=0.5, num_classes=80, num_coeffs=32):
    
    start_time_cpu = time.time()
    
    # ----------------------------------------------------
    # LOGICA COMPLETA PE CPU (NUMPY) PENTRU STABILITATE
    # ----------------------------------------------------
    
    # Primim array-uri NumPy de la TensorRT
    detections = outputs[0][0].T 
    proto_masks = outputs[1][0] 
    ...
    # 2. Conversie Boxuri (XYWH la XYXY)
    ...
    # 4. Generarea Măștilor Finale (MatMul și Sigmoid - pe CPU)
    proto_reshaped = proto_masks.reshape(num_coeffs, -1) 
    
    masks_raw = final_mask_coeffs @ proto_reshaped # <-- Operația MatMul pe CPU
    ...
    # OPTIMIZARE CV2: SCALARE CU INTER_NEAREST PENTRU VITEZĂ MAXIMĂ
    final_scaled_masks = []
    
    for mask_np in masks_final:
...
class TRT_YoloSegmentator:
    ...
    def _load_engine(self, engine_path):
        """ Încarcă și deserializează engine-ul TensorRT folosind self.runtime. """
        ...
        try:
            ...
            fout.write("--- NOTĂ: Engine-ul este probabil incompatibil. ---\n")
            raise
...
def process_frame_and_visualize(frame_original):
    ...
    # Folosim variabila globală seg_model și input_shape
    global seg_model, input_shape 
    
    final_boxes, final_class_ids, final_scaled_masks, final_scores = [], [], [], [] 
    ...
    try:
        ...
        print(f"[EROARE POST-PROCESARE] Cadru eșuat: {e}")
        fout.write(f"[EROARE POST-PROCESARE] Cadru eșuat: {e}\n")
    ...
    if len(final_boxes) > 0:
        for i, (box, class_id, score) in enumerate(zip(final_boxes, final_class_ids, final_scores)):
            ...
            log_line = (f"--- Obiectul {i + 1} ---\n"
                        f"  Clasă: {class_name} (ID: {class_id})\n"
                        f"  Încredere (Scor): {score_formatted}\n"
                        f"  Box (x1, y1, x2, y2): [{x1} {y1} {x2} {y2}]\n")
            fout.write(log_line)
            print(log_line.strip()) # Afișare consolă
...
if __name__ == "__main__":
    ...
    # 1. Verifică existența fișierului
    ...
    try:
        ...
        fout.close() # Închide fișierul de log la final
